...
if not CBLIB_VERSION or CBLIB_VERSION < 1.41 then
	...
	CBLIB_ACTIVATE_SPEC = GetSpellInfo(200749);
	
	StaticPopupDialogs["CBLIB_CONFIRM_RESET"] = {
		text = CBLIB_CONFIRM_RESET,
		button1 = YES,
		button2 = NO,
	...
	-- Reset addon
	function CBLib_ResetAddon(addonName)
	
		local dialog = StaticPopup_Show("CBLIB_CONFIRM_RESET", addonName);
		if dialog then
	...
	function CBLib_ShowBorders(self)
	...
	function CBLib_Swap(tab, val1, val2)
	...
	-- Update the bindings
	function CBLib_UpdateBindings(self, bindingPrefix)
	...
	function CBLib_ReceiveDrag(self, releaseCursor)
		...
		for i = 1, #self.availableSpells do
			if self.availableSpells[i].name == newspell then
				...
				CBLib_Swap(self.settings.buttonsOrder, self.settings.buttonsOrder[button:GetID()], i);
				CBLib_Setup(self);
				break;
			end
		end
	...
	function CBLib_IsGlyphActive(glyphId)
	...
	function CBLib_GetTalentRank(talentName, tree)
	...
	-- Show/hide a spell
	function CBLib_ToggleSpell(self, bar, idx)
	...
	function CBLib_UpdateState(self)
	...
	function CBLib_Button_SetTooltip(self)
	...
	function CBLib_StartTimer(self, guid, spellid)
	...
	function CBLib_ResetTimer(self, pos)
	...
	-- Bar Dropdown
	function CBLib_BarDropDown_OnLoad(self)
		UIDropDownMenu_Initialize(self, CBLib_BarDropDown_Initialize, "MENU");
		UIDropDownMenu_SetButtonWidth(self, 20);
		UIDropDownMenu_SetWidth(self, 20);
	end
	
	function CBLib_BarDropDown_Initialize(frame, level, menuList)
		...
		if UIDROPDOWNMENU_MENU_LEVEL == 2 then
			...
			if UIDROPDOWNMENU_MENU_VALUE == "position" then
				...
				for value, text in pairs(CBLIB_POSITIONS) do
		...
		if bar.menuHooks then
			...
			info.swatchFunc = CBLib_BarDropDown_SetBackGroundColor;
			info.func = UIDropDownMenuButton_OpenColorPicker;
			info.hasOpacity = 1;
			info.opacityFunc = CBLib_BarDropDown_SetOpacity;
			info.cancelFunc = CBLib_BarDropDown_CancelColorSettings;
			UIDropDownMenu_AddButton(info);
		end
	
	end
	
	function CBLib_BarDropDown_SetBackGroundColor()
	...
	function CBLib_BarDropDown_SetOpacity()
	...
	function CBLib_BarDropDown_CancelColorSettings(previous)
		local bar = UIDropDownMenu_GetCurrentDropDown():GetParent();
	...
	function CBLib_BarDropDown_Show(self, button)
	...
	function CBLib_UnitHasBuff(unit, name)
